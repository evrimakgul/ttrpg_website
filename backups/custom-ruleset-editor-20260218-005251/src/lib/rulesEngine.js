...
function normalizeSpellEffect(effect, effectIndex) {
  ...
  return {
    ...
    targetFieldId: safeTrim(effect?.targetFieldId),
...
function normalizeSpellGroup(group, groupIndex) {
  const tier = ALLOWED_SPELL_GROUP_TIERS.has(group?.tier) ? group.tier : "t1";
  return {
    id: safeTrim(group?.id) || `spell_group_${groupIndex + 1}`,
    name: safeTrim(group?.name) || `Spell Group ${groupIndex + 1}`,
    tier,
    tierCustomLabel: tier === "custom" ? safeTrim(group?.tierCustomLabel) || null : null,
...
export function validateRulesetSchema(inputSchema) {
  ...
  schema.boxes.forEach((box, index) => {
    ...
    if (box.type === "field") {
      ...
      if (box.isXpPool) {
        xpPoolFieldCount += 1;
        if (!isNumberField) {
          errors.push(`boxes[${index}]: isXpPool requires fieldType 'number'.`);
        }
      }
  ...
  schema.spellGroups.forEach((group, index) => {
    ...
    if (group.tier === "custom" && !group.tierCustomLabel) {
    ...
    sortedLevels.forEach((level, sortedIndex) => {
      ...
      level.effects.forEach((effect, effectIndex) => {
        if (!effect.targetFieldId) {
          errors.push(
            `spells[${index}].levels[${sortedIndex}].effects[${effectIndex}]: targetFieldId is required.`
          );
        } else {
          const target = fieldById.get(effect.targetFieldId);
          if (!target || target.fieldType !== "number") {
            errors.push(
              `spells[${index}].levels[${sortedIndex}].effects[${effectIndex}]: targetFieldId '${effect.targetFieldId}' must reference a number field.`
            );
          }
        }
...
export function evaluateRulesetRuntime(schemaInput, inputValues = {}, actorRole = "player") {
  ...
  fieldById.forEach((field, fieldId) => {
    ...
    if (!xpPoolFieldId && field.fieldType === "number" && field.isXpPool) {
